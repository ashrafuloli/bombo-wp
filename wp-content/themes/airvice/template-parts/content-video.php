...
$airvice_video_url = function_exists('get_field') ? get_field( 'fromate_style' ) : NULL;

if( is_single() ): ?>
<article id="post-<?php the_ID(); ?>" <?php post_class('ablog ablog-4 mb-55 format-video'); ?>>
        <?php if( has_post_thumbnail() ): ?>
        <div class="ablog__img ablog__img4">
            <a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('full', array('class' => 'img-responsive' )); ?></a>
            <?php if(!empty($airvice_video_url)) : ?>
            <div class="avideo-btn-4">
                <div class="blog__play p-absolute">
                    <div class="avideo-btn play_btn">
                        <a href="<?php print esc_url($airvice_video_url); ?>" class="venobox" data-autoplay="true" data-vbtype="video" ><i class="fal fa-play"></i></a>
                    </div>
                </div>
            </div>
            <?php endif; ?>
...
<?php
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('ablog ablog-4 mb-30 format-video'); ?>>
        <?php if( has_post_thumbnail() ): ?>
        <div class="ablog__img ablog__img4">
            <a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('full', array('class' => 'img-responsive' )); ?></a>
            <?php if(!empty($airvice_video_url)) : ?>
